...
@interface Level10GameObjects (PrivateMethods)
...
@implementation Level10GameObjects
...
- (void) instantiateGameObjects
{
    ...
    //floater
    self.floater = [[(Floater *)[Floater alloc] initWithPos:ccp(screenSize.width*0.35, screenSize.height*0.5) move:DOWN AndWorld:world] autorelease];
    ...
    //pipe2        
    self.pipe2 = [[(Pipe *)[Pipe alloc] initWithPos:ccp(screenSize.width*0.78, screenSize.height*0.5) shape:PIPE1 AndWorld:world] autorelease];    
...
{    
    ...
    if (floater.startContactWithBabyHamster){
        ...
        [floater autoMoveWithDistance:screenSize.height*0.23];
    }
