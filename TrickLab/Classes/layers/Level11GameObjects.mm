...
@interface Level11GameObjects (PrivateMethods)
...
@implementation Level11GameObjects
...
- (id) initWithLevelNumber:(int)levelSel
{
    if ((self=[super init])) {
        ...
        //level background
        windowSprite = [CCSprite spriteWithFile:WINDOW_DAY_SPRITE];
...
{
    //baby hamster
    self.babyHamster = [[(BabyHamster *)[BabyHamster alloc] initWithPos:ccp(screenSize.width*0.70, screenSize.height*0.85) level:levelSelected AndWorld:world] autorelease];
    ...
    //mother hamster
    self.motherHamster = [[(MotherHamster *)[MotherHamster alloc] initWithPos:ccp(screenSize.width*0.87, screenSize.height*0.13)] autorelease];
    [self addChild:motherHamster.sprite z:MOTHERHAMSTERTAG tag:MOTHERHAMSTERTAG];
    
    //cage
    self.cage = [[(Cage *)[Cage alloc] initWithPos:ccp(screenSize.width*0.70, screenSize.height*0.87) AndWorld:world] autorelease];
    [self addChild:cage.sprite z:CAGEFRONTTAG tag:CAGEFRONTTAG];
    [self addChild:cage.spriteBack z:CAGEBACKTAG tag:CAGEBACKTAG];  
    
    //blower1
    self.blower1 = [[(Blower *)[Blower alloc] initWithPos:ccp(screenSize.width*0.1, screenSize.height*0.55) Angular:0 Range:screenSize.width*0.5] autorelease];
    [self addChild:blower1.sprite z:BLOWERTAG tag:BLOWERTAG];
    
    //blower2
    self.blower2 = [[(Blower *)[Blower alloc] initWithPos:ccp(screenSize.width*0.1, screenSize.height*0.46) Angular:0 Range:screenSize.width*0.5] autorelease];
    [self addChild:blower2.sprite z:BLOWERTAG tag:BLOWERTAG];
    
    //blower3
    self.blower3 = [[(Blower *)[Blower alloc] initWithPos:ccp(screenSize.width*0.3, screenSize.height*0.85) Angular:-90 Range:screenSize.width*0.5] autorelease];
    [self addChild:blower3.sprite z:BLOWERTAG tag:BLOWERTAG];

    
    //pipe2
    self.pipe2 = [[(Pipe *)[Pipe alloc] initWithPos:ccp(screenSize.width*0.70, screenSize.height*0.75) shape:PIPE2 AndWorld:world] autorelease];    
    [self addChild:pipe2.sprite z:PIPETAG tag:PIPETAG];
    
    //pipe3
    self.pipe3 = [[(Pipe *)[Pipe alloc] initWithPos:ccp(screenSize.width*0.46, screenSize.height*0.45) shape:PIPE3 AndWorld:world] autorelease];    
    [self addChild:pipe3.sprite z:PIPETAG tag:PIPETAG];        
    
    //cradle
    self.cradle = [[(Cradle *)[Cradle alloc] initWithPos:ccp(screenSize.width*0.70, screenSize.height*0.08) AndWorld:world] autorelease];
    ...
    //carrot
    self.carrot = [[(Carrot *)[Carrot alloc] initWithPos1:ccp(screenSize.width*0.3, screenSize.height*0.70) Pos2:ccp(screenSize.width*0.65, screenSize.height*0.62) Pos3:ccp(screenSize.width*0.65, screenSize.height*0.38)] autorelease];
...
- (void)ccTouchesBegan:(NSSet *)touches withEvent:(UIEvent *)event 
{
    ...
    for (UITouch *touch in touches) {
        ...
        else if (CGRectContainsPoint(blower3.sprite.boundingBox, touchPoint)){
            //blower3
            [babyHamster checkInBlowerRange:blower3];
            CCAction *blowerAction = [[CCAnimationCache sharedAnimationCache] actionWithAnimate:[blower3 startAnimWithTag:BLOW]];
            [blower3.sprite runAction:blowerAction];
            [blower3 playParticleEffect];
        }
